//! # Requirements <https://picklenerd.github.io/pngme_book/chapter_3.html>
...
//! 6. Required methods:
//!     6.1 fn from_chunks(chunks: Vec<Chunk>) -> Png
//!     6.2 fn append_chunk(&mut self, chunk: Chunk)
//!     6.3 fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk>
//!     6.4 fn header(&self) -> &[u8; 8]
//!     6.5 fn chunks(&self) -> &[Chunk]
//!     6.6 fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk>
//!     6.7 fn as_bytes(&self) -> Vec<u8>
...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        let Some(pos) = self
            .chunks
            .iter()
            .position(|chunk| chunk.chunk_type().to_string() == chunk_type) else {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        Self::STANDARD_HEADER
            ...
            .chain(
                self.chunks
                    .iter()
                    .flat_map(|chunk| chunk.as_bytes().iter().copied().collect::<Vec<u8>>()),
            )
            .into_iter()
            .collect()
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> std::result::Result<Self, Self::Error> {
        ...
        while index < value.len() {
            let Ok(chunk) = Chunk::try_from(&value[index..]) else {
                index += 1 ;
                continue;
            };

            index += (4 + 4 + chunk.length() + 4) as usize;
            chunks.push(chunk);
        }
